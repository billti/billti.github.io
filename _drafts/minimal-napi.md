...
native modules using the NAPI API. It is used here mainly to fetch the header files and import library
...
There is a problem with the above module. The `node.lib` library is links to tells the program it will
find the NAPI functions it needs in the `node.exe` module. This can be seen by running `dumpbin /dependents /imports x64\vsnapi.node`
...
A solution to the above problem is to explicitly load the NAPI module and functions using the Win32 APIs
...
extension. GetProcAddress has the benefit of feature detection via late binding to NAPI functions,
...
this approach NAPI Node.js modules can be built with only two header files (currently) from Node.js:
`node_api.h` and `node_api_types.h`.

This does mean each NAPI function is called through an indirect pointer however. The below code shows
...
bool LoadNapiFunctions()
{
  ...
  // Macro to simplify retrieving of delay loaded NAPI functions. Keep format in sync with DECL_NAPI_IMPL in napi.h
...
The declarations and definitions for the NAPI function points are in a header file as below:
...
The `LoadNapiFunctions` function is called at initialization in `DllMain`, and the NAPI APIs are called
...
# TODO
  ...
  - Show `napi.h` first with header above, then use it in LoadNapiFunctions .cc file. (main.cc)
  - Update build.bat to not use node.lib
  - Implement things like strlen, new, and malloc using the OS APIs.
